...
import org.locationtech.geomesa.kafka.{KafkaDataStoreFactoryParams => KDSP}
...
class PutGeoMesaKafka extends AbstractGeoIngestProcessor {
  ...
  override protected def getDataStore(context: ProcessContext): DataStore = {
    DataStoreFinder.getDataStore(KdsNifiProps.map { p =>
      p.getName ->  {
        val strVal = context.getProperty(p.getName).getValue
        KdsGTProps.find(_.getName == p.getName).head.getType match {
          case x if x.isAssignableFrom(classOf[java.lang.Integer]) => strVal.toInt
          case x if x.isAssignableFrom(classOf[java.lang.Long])    => strVal.toLong
          case x if x.isAssignableFrom(classOf[java.lang.Boolean]) => strVal.toBoolean
          case _                                                   => strVal
        }
      }
    }.toMap.asJava)
